...
signal s_ajuste_brilho: STD_LOGIC_VECTOR (23 downto 0);
...
--#----------------------------------FILTRO BINARIZAÇÃO(0001)---------------------------------------------------------#
process(s_mem_din, s_ajuste_brilho)
begin 
        brilho3 <= to_integer(unsigned(brilho));
        s_red_color <= (to_integer( unsigned(s_mem_din(23 downto 16) ) ) * brilho3) / 128;
        s_green_color <= (to_integer( unsigned(s_mem_din(15 downto 8) ) ) * brilho3) / 128;
        s_blue_color <= (to_integer( unsigned(s_mem_din(7 downto 0) ) ) * brilho3) / 128;
        s_ajuste_brilho <= std_logic_vector(to_unsigned(s_red_color, 8))& std_logic_vector(to_unsigned(s_green_color, 8)) & std_logic_vector(to_unsigned(s_blue_color, 8));
