...
Port ( clk : in STD_LOGIC;
       ...
       brilho: out std_logic_VECTOR (15 downto 0)
     );
end controller_FSM;
...
--Estados da FSM
type state_type is (init, wait_1, wait_0, wait_2,wait_3, decode, espera, black, binarization, negative, shades_of_gray, RGB, RGB1, RGB2, img_normal, remdimantion, salt_chili, state_salt, state_chili, exec_sobe_brilho, exec_baixa_brilho);  --type of state machine.
...
signal const_331: STD_LOGIC_VECTOR (11 downto 0):= x"14b";
...
signal s_control_brilho: std_logic := '0';
signal brilho2 : integer := 128;          -- Sinal para aumentar o brilho; 
...
begin
  ...
  case current_s is
   ...
   -- toggle_rgb <= '1';
    ld_con <= '1';
    d_con <= "000000000000000";
    ld_random <= '1';
    d_random <= "000000000000001";  
    when wait_1 =>
       --s_exec_loop <= '0';
      if(start='1') then
        next_s <= wait_0;
...
--#----------------------------------FILTRO SAL E PIMENTA(1000)-----------------------------------------------#
        ...
        when state_salt => 
          ...
          --  habilitar_contador <= '1';
            ...
            ld_con <= '0';
            ld_random <= '1';
            aux_salt_chili <= (q_random * const_331);
        ...
        when state_chili =>
            ...
            ld_con <= '1';
            ld_random <= '1';
            aux_salt_chili <= (s_mem_addr * const_331);
            d_random <= aux_salt_chili(14 downto 0);
...
--#----------------------------------FILTRO brilho(0111)-----------------------------------------------#
        ...
        when exec_baixa_brilho =>
                        ...
                        brilho <= std_logic_vector(to_unsigned(brilho2, 16));
                        if (rst = '1') then 
                            next_s <= init;
                        end if;
                    s_control_brilho <= '1';                
...
process (clk, s_control_brilho)
begin
 --if (rst='0') then
    if (rising_edge(clk)) then
        if(s_control_brilho = '1') then
        ...
        if (contador_de_pulsos_brilho >= 5000000) then 
             ...
             if(s_exec_start='1' and op="0111" ) then
             
                if(brilho2 > 0 and s_brilho_op = '0') then 
                ...
                if(brilho2 < 128 and s_brilho_op = '1') then 
...
reg_random: entity work.REG_random 
    generic map (N=> 15)
    port map(clk=>clk, 
             rst=>rst, 
             ld=> ld_random, 
...
s_mem_addr <= pixel_addr when ((s_exec_start='1') and (mux_salt_chili = '0') and s_exec_loop = '0') else 
              q_random   when ((mux_salt_chili = '1') and (s_exec_start = '0') and s_exec_loop = '0') else
              s_aux_mem_addr(14 downto 0) when (s_exec_loop = '1')             else 
              (others => '0');
